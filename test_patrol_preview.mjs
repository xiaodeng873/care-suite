...
function generatePatrolRoundsHtml(input) {
...
.page-label { text-align: center; font-weight: 600; color: #374151; }`;
  ...
  const pageFragment = renderPageFragment(1, 1, input);
...
<title>${esc(input.facilityName ?? '巡房記錄')}</title>
<style>${css}</style>
</head>
<body>
${pageFragment}
...
// Generate sample patrol data
const rounds = [];
for (let i = 1; i <= 3; i++) {
  for (let tIdx = 0; tIdx < 6; tIdx++) {
    rounds.push({
      patrol_date: `2026-07-0${i}`,
...
const html = generatePatrolRoundsHtml({
  facilityName: '善頤(福群)護老院',
  bedNumber: 'A-101',
  year: 2026,
  month: 7,
...
fs.writeFileSync('/workspaces/care-suite/patrol_preview_beautified.html', html);
console.log('✓ Generated patrol_preview_beautified.html');
