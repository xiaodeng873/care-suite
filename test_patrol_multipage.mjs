...
function renderPageFragment(pageNum, totalPages, input) {
  ...
  for (let d = dayStart; d <= dayEnd; d++) {
    ...
    const rows = TIME_SLOTS.map((ts, idx) => {
      const r = map.get(dd)?.get(ts) ?? null;
    ...
    const card = `<div class="card">
      <div class="card-hd">
        <div class="card-date">${esc(dateLabel)}</div>
      </div>
      <div class="card-body">
  ...
  const halfLabel = dayStart === 1 ? '上半月' : '下半月';
  const pageLabel = totalPages > 1 ? `巡房記錄 共${pageNum}/${totalPages}頁 - ${esc(halfLabel)}` : `巡房記錄 - ${esc(halfLabel)}`;
...
function generateDocumentFromPages(pages) {
...
.page-label { text-align: center; font-weight: 600; color: #374151; }`;
  ...
  const fragments = pages.map((p, idx) => renderPageFragment(idx + 1, totalPages, p)).join('\n');
...
// Generate sample patrol data across two pages (上/下半月)
const rounds = [];
for (let i = 1; i <= 30; i++) {
  const day = String(i).padStart(2, '0');
  for (let tIdx = 0; tIdx < 6; tIdx++) {
    rounds.push({
      patrol_date: `2026-07-${day}`,
...
// Create two page inputs: upper half (1-16) and lower half (17-30)
const pages = [
  {
    facilityName: '善頤(福群)護老院',
    bedNumber: 'A-101',
...
const html = generateDocumentFromPages(pages);
fs.writeFileSync('/workspaces/care-suite/patrol_preview_multipage.html', html);
console.log('✓ Generated patrol_preview_multipage.html (2 pages: upper/lower half month)');
