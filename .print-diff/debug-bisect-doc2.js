...
(async () => {
  ...
  // 抽出 header / content / footer 三段
  const headerHtml = body.match(/<div class="page-header">([\s\S]*?)<\/div>\s*<div class="page-content">/)[1];
  const tableHtml = body.match(/<table class="main-table">[\s\S]*?<\/table>/)[0];

  const page = (inner) => `<div class="page">${inner}</div>`;
  const variants = {
    '只有header': page(`<div class="page-header">${headerHtml}</div>`),
    '只有table(page-content包)': page(`<div class="page-content">${tableHtml}</div>`),
    'header+空白content+footer': page(`<div class="page-header">${headerHtml}</div><div class="page-content"></div><div class="footer"><div class="page-num">6</div><div class="doc-code">X</div></div>`),
    '只有空白page': page(''),
  };
  ...
  for (const [name, b] of Object.entries(variants)) {
    ...
    const pdfBuf = await page2.pdf({ format: 'A4', printBackground: true });
