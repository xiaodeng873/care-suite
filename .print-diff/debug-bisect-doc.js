...
(async () => {
  ...
  const variants = {
    '原樣': body,
    '去掉footer': body.replace(/<div class="footer">[\s\S]*?<\/div>\s*<\/div>$/, '</div>'),
    '去掉inputs': body.replace(/<input[^>]*>/g, ''),
    '主表格換成空白div': body.replace(/<table class="main-table">[\s\S]*?<\/table>/, '<div style="height:890px"></div>'),
  };
