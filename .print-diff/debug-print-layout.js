...
(async () => {
  ...
  const variants = {
    wrapped: `<div class="print-doc-0-0" style="padding: 5mm 6.35mm; box-sizing: border-box;">${body}</div>`,
    bare: body,
  };
  ...
  for (const [name, inner] of Object.entries(variants)) {
    ...
    const m = await page.evaluate(() => {
      ...
      const kids = Array.from(el.children).map(c => ({ cls: c.className, h: Math.round(c.getBoundingClientRect().height * 100) / 100 }));
      return {
        pageH: Math.round(el.getBoundingClientRect().height * 100) / 100,
        pageHPx: el.getBoundingClientRect().height,
        kids,
        tableH: el.querySelector('table.main-table').getBoundingClientRect().height,
