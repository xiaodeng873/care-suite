...
const docs = [];
for (const [name, p] of [
  ['nursing', await (await bundleNode('apps/web/src/utils/patientLogNursingTreatmentGenerator.ts')).generatePatientLogNursingTreatmentHtml(
    [{ id: 'b', patient_id: 999, log_date: '', log_type: '其他', content: '', recorder: '' }],
    [{ 院友id: 999, 中文姓名: '測試院友', 中文姓氏: '測', 中文名字: '試院友', 床號: 'A103-2', 性別: '女', 出生日期: '1929-01-01', 在住狀態: '在住' }],
    ['b'])],
]) {
...
fs.writeFileSync(path.join(__dirname, 'repro-nursing-debug.pdf'), pdfBuf);
await browser.close();
