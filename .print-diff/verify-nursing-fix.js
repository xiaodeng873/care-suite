// 驗證：34 列後 direct 及 grouped 兩條路徑都應為 1 頁
...
(async () => {
  ...
  for (const n of [34, 35, 68, 69]) {
    ...
    const variants = {
      direct: `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@page{size:A4;margin:5mm 0.25in;}</style><style>${styles}</style></head><body>${body}</body></html>`,
