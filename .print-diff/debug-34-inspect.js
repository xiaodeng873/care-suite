...
(async () => {
  ...
  const lay = await page.evaluate(() => {
    ...
    const nat = el.querySelector('.page-header').getBoundingClientRect().height
      + el.querySelector('.page-content').getBoundingClientRect().height
      + el.querySelector('.footer').getBoundingClientRect().height;
    return {
      pageHeightPx: el.getBoundingClientRect().height,
      naturalSumPx: nat,
      headerPx: el.querySelector('.page-header').getBoundingClientRect().height,
      contentPx: el.querySelector('.page-content').getBoundingClientRect().height,
      footerPx: el.querySelector('.footer').getBoundingClientRect().height,
      bodyScrollPx: document.body.scrollHeight,
    };
  });
  console.log(JSON.stringify(lay, null, 1));
  await browser.close();
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    console.log(`page ${p}:`, items.slice(0, 10).join('|'));
  }
