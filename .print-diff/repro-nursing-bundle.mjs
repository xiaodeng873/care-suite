...
doc.getPages().forEach((p, pi) => {
  ...
  if (!data.trim()) { console.log(`--- PDF page ${pi + 1}: no content stream ---`); return; }
  ...
  console.log(`  black strokes: ${strokes}, x range: ${strokes ? `${toMmX(minX)} ~ ${toMmX(maxX)}mm` : 'n/a'}`);
