...
(async () => {
  ...
  for (const h of [287, 286, 285, 284]) {
    ...
    const styles = (adjusted.match(/<style[^>]*>([\s\S]*?)<\/style>/gi) || []).map(t => t.replace(/^<style[^>]*>|<\/style>$/gi, '')).join('\n').replace(/@page[^{]*\{[^}]*\}/g, '');
    const body = adjusted.match(/<body[^>]*>([\s\S]*?)<\/body>/i)[1].trim();
    ...
    console.log('grouped 34rows min-height', h, 'pdfPages=', pdfDoc.getPageCount());
    await page.close();
  }
  await browser.close();
