...
const logoHtml = printPageLogo.injectPageLogo(nursingHtml, redPng);
const pm = logoHtml.match(/@page[^{]*\{[^}]*\}/);
console.log('=== 第一個 @page 區塊（injectPageLogo 後）===\n', pm && pm[0]);

// 醫生診治記錄（雙面雙頁）：直接 combine 模板（唔注入數據，純版式驗證）
const fs2 = await import('fs');
const dvTemplate = fs2.readFileSync(path.join(root, 'upload/doc_html/醫生診治記錄.html'), 'utf-8');
const baseProc = await bundleNode('apps/web/src/utils/docHtmlGenerators/baseTemplateProcessor.ts');
const dvHtml = printPageLogo.injectPageLogo(baseProc.combineDocHtmlDocuments([dvTemplate, dvTemplate]), redPng);

// ---- 照 printGroupedHtml 組裝 ----
const pages = [logoHtml, dvHtml];
const config = extractPageConfig(logoHtml);
