...
const patient = {
  ...
  prescriptions: [
    { medication_name: 'INSULIN GLARGINE (LANTUS SOLOSTAR) PREFILLED PEN 100U/ML 3ML', administration_route: '皮下注射', dosage_form: '注射劑', meal_timing: '早餐前', frequency_type: 'daily', daily_frequency: 1, dosage_amount: 1, dosage_unit: '次', start_date: '2026-09-18', inspection_rules: [{ vital_sign_type: '血糖值', condition_operator: 'lte', condition_value: 4, action_if_met: 'warning_only' }] },
...
const info = await page.evaluate(() => {
  ...
  return rows.map(r => {
    ...
    return {
      ...
      cells: Array.from(r.querySelectorAll('td')).map(td => td.textContent.slice(0, 30)),
    };
  });
});
