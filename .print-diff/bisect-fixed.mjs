...
const variants = {
  full: () => true,
  // 淨留第 3 份（financial proxy，0 margin）
  only3: (cls) => cls === 'print-doc-0-2',
  // 淨留第 1 份（mt 0 mb 5）
  only1: (cls) => cls === 'print-doc-0-0',
};
...
for (const [name, keep] of Object.entries(variants)) {
  ...
  if (name !== 'full') {
    await page.evaluate((keepName) => {
      document.querySelectorAll('[class*="print-doc-"]').forEach((w) => {
        const cls = w.className;
        const keep = keepName === 'only3' ? cls === 'print-doc-0-2' : cls === 'print-doc-0-0';
        if (!keep) w.remove();
      });
    }, name);
  }
  const pdfPath = `C:/Users/Admin/Desktop/care-suite/.print-diff/bisect-${name}.pdf`;
  await page.pdf({ path: pdfPath, preferCSSPageSize: true, printBackground: true });
  await browser.close();
  console.log(name, 'page1 circles:', JSON.stringify(await measure(pdfPath)));
}
