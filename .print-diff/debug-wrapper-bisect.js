...
(async () => {
  ...
  const variants = {
    'wrapper無樣式': `<div class="print-doc-0-0">${body}</div>`,
    'wrapper只有padding': `<div class="print-doc-0-0" style="padding: 5mm 6.35mm;">${body}</div>`,
    'wrapper只有box-sizing': `<div class="print-doc-0-0" style="box-sizing: border-box;">${body}</div>`,
    'wrapper padding+bsb': `<div class="print-doc-0-0" style="padding: 5mm 6.35mm; box-sizing: border-box;">${body}</div>`,
    'wrapper 純文字外再加一個空div': `<div class="print-doc-0-0" style="padding: 5mm 6.35mm; box-sizing: border-box;">${body}</div><div></div>`,
  };

  const browser = await chromium.launch({ executablePath: CHROME });
  for (const [name, inner] of Object.entries(variants)) {
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>html,body{margin:0;padding:0;}@page{size:A4 portrait;margin:0;}</style><style>${styles}</style></head><body>${inner}</body></html>`;
