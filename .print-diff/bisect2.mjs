...
const run = async (name, fn) => {
  ...
  const pdfPath = `C:/Users/Admin/Desktop/care-suite/.print-diff/bisect2-${name}.pdf`;
  await page.pdf({ path: pdfPath, preferCSSPageSize: true, printBackground: true });
  await browser.close();
  console.log(name, JSON.stringify(await measure(pdfPath)));
};

// 1. 淨留 financial proxy + 刪晒佢嘅 scoped style
await run('only3-nostyle', () => {
  document.querySelectorAll('[class*="print-doc-"]').forEach((w) => {
    if (w.className !== 'print-doc-0-2') w.remove();
  });
  document.querySelectorAll('style').forEach((st) => {
    if ((st.textContent || '').includes('.print-doc-0-2')) st.remove();
  });
});
