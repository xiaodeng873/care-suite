// 35 列經 printGroupedHtml 模擬，測試不同 .page min-height 的頁數
...
(async () => {
  ...
  // 模擬 printGroupedHtml：抽 @page、用 padding 還原 margin（不 scope 選擇器，單份文件效果相同）
  const stripPage = (page) => {
    const styles = (page.match(/<style[^>]*>([\s\S]*?)<\/style>/gi) || [])
      .map(t => t.replace(/^<style[^>]*>|<\/style>$/gi, ''))
      .join('\n')
      .replace(/@page[^{]*\{[^}]*\}/g, '');
    const body = page.match(/<body[^>]*>([\s\S]*?)<\/body>/i)?.[1]?.trim() ?? page.trim();
    return { styles, body };
  };

  const browser = await chromium.launch({ executablePath: CHROME });
  for (const h of [287, 286, 285, 284]) {
    const adjusted = docHtml.replace('min-height: 287mm', `min-height: ${h}mm`);
    const { styles, body } = stripPage(adjusted);
...
<div class="print-doc-0-0" style="padding: 5mm 6.35mm; box-sizing: border-box;">${body}</div>
    ...
    console.log('grouped min-height', h, 'pdfPages=', pdfDoc.getPageCount());
    await page.close();
  }
  await browser.close();
