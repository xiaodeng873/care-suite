...
doc.getPages().forEach((p, pi) => {
  ...
  let minTopPt = Infinity, maxBottomPt = -Infinity; const grayBoxes = [];
  const images = [];
  let i = 0;
    ...
    if (t === 'q') { stack.push(ctm); }
      ...
      if (pathPts.length && (isBlack || isGray)) {
        if (isBlack) strokes++; else { grayStrokes++;
          const xs = pathPts.map(p=>xf(ctm,p[0],p[1])[0]*MM), ys = pathPts.map(p=>(height-xf(ctm,p[0],p[1])[1])*MM);
          grayBoxes.push([Math.min(...xs).toFixed(1),Math.max(...xs).toFixed(1),Math.min(...ys).toFixed(1),Math.max(...ys).toFixed(1)].join(',')); }
  ...
  console.log('  gray boxes [x1,x2,y1,y2]:', grayBoxes.join(' | '));
});
console.log('done');
