...
(async () => {
  ...
  for (const n of [34, 35, 68]) {
    ...
    const variants = {
      '底部留1px': 'padding: 5mm 6.35mm calc(max(5mm - 1px, 0px)) 6.35mm; box-sizing: border-box;',
      '原樣': 'padding: 5mm 6.35mm; box-sizing: border-box;',
    };
    const out = [];
    for (const [name, wrapStyle] of Object.entries(variants)) {
      ...
      out.push(`${name}=${pdfDoc.getPageCount()}頁`);
      await page.close();
    }
    console.log(`rows=${n}`, out.join(' '));
  }
  await browser.close();
