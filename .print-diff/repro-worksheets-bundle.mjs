...
// ---- 生成四個觀察表 html（stub 院友 999，無記錄 → 每份一張空白頁 ×2 雙面）----
const bp = await bundleNode('apps/web/src/utils/bloodPressureRecordWorksheetGenerator.ts');
const temp = await bundleNode('apps/web/src/utils/temperatureRecordWorksheetGenerator.ts');
const bw = await bundleNode('apps/web/src/utils/bodyweightRecordWorksheetGenerator.ts');
const glu = await bundleNode('apps/web/src/utils/glucoseRecordWorksheetGenerator.ts');
...
const opts = { includeData: false };
const docs = [];
for (const [name, fn] of [
  ['vital_signs', bp.generateBloodPressureRecordHtml],
  ['temperature', temp.generateTemperatureRecordHtml],
  ['bodyweight', bw.generateBodyweightRecordHtml],
  ['glucose', glu.generateGlucoseRecordHtml],
]) {
  const html = await fn('2026-01-01', '2026-12-31', [999], opts);
  const containers = (html.match(/class="container"/g) || []).length;
  console.log(name, 'containers:', containers, 'punch:', html.includes('punch-guide-fixed'));
  docs.push(printPageLogo.injectPageLogo(html, redPng));
}
...
fs.writeFileSync(path.join(__dirname, 'repro-worksheets.pdf'), pdfBuf);
await browser.close();
