...
const generators = [
  ...
  { file: '../apps/web/src/utils/docHtmlGenerators/nursingAssessmentGenerator.ts', fn: 'generateNursingAssessmentHtml', name: 'nursing_assessment' },
];
...
(async () => {
  for (const g of generators) {
    try {
      ...
      fs.writeFileSync(path.join(outDir, `${g.name}_test.html`), html);
      console.log('generated', g.name);
    } catch (e) {
      console.error('failed', g.name, e.message, e.stack);
    }
  }
})();
