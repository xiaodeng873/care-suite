...
const generateDataRows = (records: PatientActivityRecord[]): string => {
  return records.map(record => {
    ...
    return `<tr style="height: 30px;">${cells}</tr>`;
  }).join('');
};
...
const pageBlock = (patient: Patient, pageRecords: PatientActivityRecord[], pageIndex: number, totalPages: number, facilityName: string): string => {
  ...
  const emptyRows = Array(emptyRowCount).fill(`<tr style="height: 30px;">${'<td></td>'.repeat(19)}</tr>`).join('');
...
export const generateActivityRecordPrintFormHtml = (
  ...
  const pages = allPages.map((page, idx) => pageBlock(page.patient, page.pageRecords, idx, totalPages, facilityName)).join('');
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .title-section h2 { margin: 2px 0 0 0; font-size: 20px; font-weight: bold; display: inline-block; border-bottom: 1.5px solid black; padding-bottom: 2px; }
  .user-info { display: flex; justify-content: space-between; margin-bottom: 2px; font-weight: bold; font-size: 13px; }
  .db-line-input { border: none; border-bottom: 1px solid black; background: transparent; font-family: inherit; font-size: 13px; padding: 0 5px; }
  ...
  .vertical-text { writing-mode: vertical-rl; text-orientation: mixed; height: 85px; display: flex; align-items: center; justify-content: center; line-height: 1; margin: 0 auto; letter-spacing: 0px; }
  .db-checkbox { width: 12px; height: 12px; cursor: default; margin: 0; }
  .db-text-cell { width: 100%; height: 100%; border: none; background: transparent; font-family: inherit; font-size: 10px; text-align: center; outline: none; }
  .db-textarea { width: 100%; height: 100%; border: none; background: transparent; font-family: inherit; font-size: 9px; resize: none; overflow: hidden; display: block; padding: 1px; box-sizing: border-box; line-height: 1.1; }
  .notes-section { font-size: 9.5px; margin-top: 2px; line-height: 1.15; }
  .notes-heading { font-weight: bold; font-size: 10px; margin-bottom: 1px; }
  .note-item { display: flex; margin-bottom: 0px; }
...
export const printActivityRecordForm = async (
  ...
  const html = generateActivityRecordPrintFormHtml(patients, recordsByPatient, settings.facilityNameZh);
