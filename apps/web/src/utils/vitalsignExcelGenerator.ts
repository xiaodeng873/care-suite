...
export const exportVitalSignsToExcel = async (
  ...
  try {
    ...
    Object.entries(groupedRecords).forEach(([key, recordGroup]) => {
      ...
      if (patient) {
        sheetsConfig.push({
          name: `${getPrintBedNumber(patient)}${patient.中文姓氏}${patient.中文名字}生命表徵觀察記錄表`,
    ...
    const finalFilename = filename ||
      (sheetsConfig.length === 1
        ? `${getPrintBedNumber(sheetsConfig[0].patient)}_${sheetsConfig[0].patient.中文姓氏}${sheetsConfig[0].patient.中文名字}_生命表徵觀察記錄表.xlsx`
...
const exportVitalSignsToExcelSimple = async (
  ...
  Object.entries(groupedRecords).forEach(([key, recordGroup]) => {
    ...
    const patient = patients.find(p => p.床號 === firstRecord.床號 && `${p.中文姓氏}${p.中文名字}` === `${firstRecord.中文姓氏}${firstRecord.中文名字}`);
    if (!patient) return;
    const sheetName = `${getPrintBedNumber(patient)}${patient.中文姓氏}${patient.中文名字}生命表徵觀察記錄表`;
