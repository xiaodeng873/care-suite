...
const renderTable = (cells: WeightCell[]): string => {
  ...
  const bodyRows = grid.map(row => {
    const tds = row.map((cell, setIdx) => {
      ...
      return `<td><input class="db-text-cell" value="${dateText}" readonly></td><td><input class="db-text-cell" value="${weightText}" readonly></td><td><input class="db-text-cell" value="${compareText}" readonly></td><td><div class="${noteClass}">${noteText}</div></td><td${dividerAttr}><input class="db-text-cell" value="${recorderText}" readonly></td>`;
    }).join('');
