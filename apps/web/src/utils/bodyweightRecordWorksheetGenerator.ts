...
// 每頁 2 組「日期 / 體重(Kg) / 淨改變% / 跟進行動(註) / 記錄職員」× 30 列。
...
const renderTable = (cells: WeightCell[]): string => {
  ...
  const bodyRows = grid.map(row => {
    const tds = row.map((cell, setIdx) => {
      ...
      return `<td><input class="db-text-cell" value="${dateText}" readonly></td><td><input class="db-text-cell" value="${weightText}" readonly></td><td><input class="db-text-cell" value="${compareText}" readonly></td><td><input class="db-text-cell" value="${noteText}" readonly style="text-align:left; padding-left:5px;"></td><td${dividerAttr}><input class="db-text-cell" value="${recorderText}" readonly></td>`;
    }).join('');
...
<style>
  ...
  * { box-sizing: border-box; }
  body { font-family: "DFKai-SB", "BiauKai", "標楷體", serif; margin: 0; background-color: #fff; color: #000; line-height: 1.2; } /* 唔好寫 padding:0——bundle 路徑打孔指引（padding-left:20mm）同 logo 補償靠 body padding 生效 */
