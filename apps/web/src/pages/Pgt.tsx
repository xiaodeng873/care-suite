...
import PgtModal from '../components/PgtModal';
import { fuzzyMatch, matchChineseName, matchEnglishName, matchPatientBedNumber} from '../utils/searchUtils';
...
type SortField = '院友姓名' | 'medication_end_date' | 'pgt_visit_date' | 'created_at';
...
const Pgt: React.FC = () => {
  // PGT 特例：解除站別過濾，列出所有院友
  const { patients, loading } = usePatientData();
  const { pgtRecords, deletePgtRecord, visitDates, visitDatesLoaded } = usePgt();
  const [showModal, setShowModal] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState<PgtRecord | null>(null);
  const [renewFromRecord, setRenewFromRecord] = useState<PgtRecord | null>(null);
  ...
  if (loading) {
    return <LoadingScreen pageName="PGT" />;
  }
  const filteredRecords = useMemo(() => {
    const searchTerm = deferredSearch;
    return pgtRecords.filter(record => {
  ...
  }, [pgtRecords, patients, advancedFilters, deferredSearch]);
  ...
  const sortedRecords = [...filteredRecords].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'medication_end_date':
        ...
        break;
      case 'pgt_visit_date':
        valueA = a.pgt_visit_unknown ? Number.MAX_SAFE_INTEGER : (a.pgt_visit_date ? new Date(a.pgt_visit_date).getTime() : 0);
        valueB = b.pgt_visit_unknown ? Number.MAX_SAFE_INTEGER : (b.pgt_visit_date ? new Date(b.pgt_visit_date).getTime() : 0);
        break;
      case 'created_at':
  ...
  const groupedRecords = (() => {
    ...
    const groups: { patientId: number; records: PgtRecord[] }[] = [];
  ...
  // 到診日期清單（最新 doctor_visit_schedule，與 CGAT 共用）；清單未載入前唔做不符判斷，避免誤報
  const visitDateSet = useMemo(() => new Set(visitDates), [visitDates]);
  // 記錄嘅診症日期已被刪/被改，對唔上最新 PGT 到診日期清單
  const isVisitDateMismatch = (r: PgtRecord) =>
    visitDatesLoaded && !r.pgt_visit_unknown && !!r.pgt_visit_date && !visitDateSet.has(r.pgt_visit_date);
  ...
  const handleEdit = (record: PgtRecord) => {
    setSelectedRecord(record);
    setRenewFromRecord(null);
    setShowModal(true);
  };
  const handleDelete = async (id: string) => {
    const record = pgtRecords.find(c => c.id === id);
    const patient = patients.find(p => p.院友id === record?.patient_id);
    if (confirm(`確定要刪除 ${patient?.中文姓名} 的 PGT 記錄嗎？`)) {
      try {
        setDeletingIds(prev => new Set(prev).add(id));
        await deletePgtRecord(id);
  ...
  const handleBatchDelete = async () => {
    ...
    const confirmMessage = `確定要刪除 ${selectedRows.size} 筆 PGT 記錄嗎？\n\n此操作無法復原。`;
  ...
  // ── PGT 欄位顯示 ──
  const caseTypeText = (r: PgtRecord) => r.case_type || '—';
  const reasonText = (r: PgtRecord) => {
    const parts: string[] = [];
  ...
  const pharmacyText = (r: PgtRecord) => {
    const parts: string[] = [];
    ...
    <div className="space-y-6">
      <div className="sticky top-0 bg-white z-30 py-4 border-b border-gray-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h1 className="text-2xl font-bold text-gray-900">PGT</h1>
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={async () => {
                if (selectedRows.size === 0) {
                  alert('請先選擇要列印的 PGT 記錄');
                  return;
                }
                const selected = sortedRecords.filter((r) => selectedRows.has(r.id));
                await printCgatSummary({ records: selected, patients, title: 'PGT 診症摘要' });
              }}
        ...
        <div className="card p-4">
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-gray-600">
              <span>顯示 {startIndex + 1}-{Math.min(endIndex, totalItems)} / {totalItems} 位院友 (共 {pgtRecords.length} 筆記錄)</span>
              {(searchTerm || hasAdvancedFilters()) && (
        ...
        {paginatedGroups.length > 0 ? (
            ...
            <table className="w-full min-w-[768px] divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <SortableHeader field="pgt_visit_date">PGT 到診日期</SortableHeader>
                ...
                {paginatedGroups.map(group => {
                  ...
                  return displayRecords.map((record, recordIndex) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {record.pgt_visit_unknown ? (
                          <span className="text-red-600">未知</span>
                        ) : record.pgt_visit_date && isVisitDateMismatch(record) ? (
                          <div
                            className="flex flex-col cursor-help"
                            title="此診症日期已不在最新 PGT 到診日期清單（已被刪除或更改），請編輯重新選擇日期，或改為未知"
                          >
                            <span className="text-red-600">日期不符</span>
                            <span className="text-xs text-gray-400 line-through">{formatDisplayDate(record.pgt_visit_date)}</span>
                          </div>
                        ) : record.pgt_visit_date ? (
                          <div className="flex items-center space-x-1">
                            <Calendar className="h-4 w-4 text-gray-400" />
                            <span>{formatDisplayDate(record.pgt_visit_date)}</span>
                          </div>
                        ) : (
                          <span className="text-gray-500">-</span>
                        )}
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {searchTerm || hasAdvancedFilters() ? '找不到符合條件的 PGT 記錄' : '暫無 PGT 記錄'}
            </h3>
            <p className="text-gray-600 mb-4">
              {searchTerm || hasAdvancedFilters() ? '請嘗試調整搜索條件' : '開始為院友建立 PGT 記錄'}
            </p>
            {!searchTerm && !hasAdvancedFilters() ? (
              <button
