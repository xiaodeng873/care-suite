...
export default function HygieneIndex() {
  ...
  const renderItem = ({ item }: { item: Resident }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      onPress={() =>
        router.push({ pathname: '/(app)/hygiene/[id]', params: { id: String(item.院友id) } })
      }
    >
      <View className="flex-row items-center gap-3">
        <View className={`w-8 h-8 rounded-full items-center justify-center ${item.性別 === '男' ? 'bg-blue-100' : 'bg-pink-100'}`}>
        ...
        <View>
          ...
          {item.床號 ? <Text className="text-xs text-gray-400">{item.床號}</Text> : null}
        </View>
      </View>
