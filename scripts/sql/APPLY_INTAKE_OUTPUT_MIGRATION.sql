-- ============================================
-- 修复 intake_output_records 表并添加新的 items 表
-- 执行日期: 2025-12-22
-- ============================================

-- ============================================
-- 步骤 1: 添加 time_slot 字段
-- ============================================
...
-- 删除旧策略（如果存在）
DROP POLICY IF EXISTS "Enable all for authenticated users" ON intake_items;
DROP POLICY IF EXISTS "Enable all for authenticated users" ON output_items;
...
-- 完成！
-- ============================================
-- 现在您可以使用新的动态项目结构来记录出入量数据
-- 旧的 JSONB 字段 (meals, beverages 等) 仍然保留，用于向后兼容
