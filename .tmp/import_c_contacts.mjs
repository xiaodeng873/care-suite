// 匯入 C 站院友聯絡人清單（upload/C站聯絡人清單.csv）
...
const raw = readFileSync('upload/C站聯絡人清單.csv', 'utf8').replace(/^\uFEFF/, '');
...
// 院友對照：facility_id=1，中文姓名精確匹配；重名取「在住」
...
const resolve = (name) => {
  ...
  return { pid: chosen.院友id, note: `重名${list.length}條，取${chosen.在住狀態 || '?'}` };
};
