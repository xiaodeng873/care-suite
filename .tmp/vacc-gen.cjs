...
// apps/web/src/utils/vaccinationRecordPrintGenerator.ts
var vaccinationRecordPrintGenerator_exports = {};
__export(vaccinationRecordPrintGenerator_exports, {
  generateVaccinationRecordHtml: () => generateVaccinationRecordHtml,
  printVaccinationRecord: () => printVaccinationRecord
});
module.exports = __toCommonJS(vaccinationRecordPrintGenerator_exports);
...
var BLOCK_TITLES = ["\u4E00\u3001\u6D41\u611F\u75AB\u82D7", "\u4E8C\u3001\u80BA\u708E\u93C8\u7403\u83CC\u75AB\u82D7", "\u4E09\u3001\u65B0\u51A0\u75AB\u82D7", "\u56DB\u3001\u5176\u4ED6\u75AB\u82D7"];
var CHUNK_SIZE = 15;
var classifyVaccine = (item) => {
  const s = (item || "").toLowerCase();
  if (s.includes("\u6D41\u611F")) return 0;
  if (s.includes("\u80BA\u708E")) return 1;
  if (s.includes("\u65B0\u51A0") || s.includes("covid") || s.includes("\u5FA9\u5FC5\u6CF0") || s.includes("\u79D1\u8208") || s.includes("coronavac") || s.includes("biontech")) {
    return 2;
  }
  return 3;
};
var generateVaccinationRecordHtml = (patient, records, facilityName) => {
  const blocks = [[], [], [], []];
  for (const r of records) {
    blocks[classifyVaccine(r.vaccine_item)].push(r);
  }
  blocks.forEach(
    (list) => list.sort((a, b) => (a.vaccination_date || "").localeCompare(b.vaccination_date || ""))
  );
  const patientName = patient.\u4E2D\u6587\u59D3\u540D || `${patient.\u4E2D\u6587\u59D3\u6C0F || ""}${patient.\u4E2D\u6587\u540D\u5B57 || ""}`;
  const sections = blocks.map((list, i) => {
    const rowsHtml = (rows) => rows.length === 0 ? "<tr><td></td><td></td></tr>" : rows.map((r) => `<tr><td>${escapeHtml(formatDisplayDate(r.vaccination_date))}</td><td>${escapeHtml(r.vaccine_item)}</td></tr>`).join("");
...
var printVaccinationRecord = async (patient, records) => {
