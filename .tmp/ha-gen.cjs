...
var init_dist4 = __esm({
  "node_modules/@supabase/supabase-js/dist/index.mjs"() {
    ...
    fetchWithAuth = (supabaseKey, supabaseUrl2, getAccessToken, customFetch, tracePropagationOptions, options) => {
...
function isBoltDatabaseUrl(url) {
  if (!url) return false;
  return url.includes(BOLT_DATABASE_URL);
}
function getSupabaseUrl() {
  const envUrl = import_meta.env.VITE_SUPABASE_URL;
...
function getSupabaseAnonKey() {
  const envUrl = import_meta.env.VITE_SUPABASE_URL;
  const envKey = import_meta.env.VITE_SUPABASE_ANON_KEY;
...
var import_meta, BOLT_DATABASE_URL, SUPABASE_CONFIG;
var init_supabase_config = __esm({
  "apps/web/src/config/supabase.config.ts"() {
    "use strict";
    import_meta = { env: { VITE_SUPABASE_URL: "https://x.supabase.co", VITE_SUPABASE_ANON_KEY: "x", DEV: false } };
    BOLT_DATABASE_URL = "kzuhukvdhnuphuilkdmd";
    SUPABASE_CONFIG = {
...
var supabaseUrl, supabaseAnonKey, validation, DB_TOKEN_KEY, noTokenWarnCount, dbFetch, supabase;
var init_supabase = __esm({
  "apps/web/src/lib/supabase.ts"() {
    ...
    DB_TOKEN_KEY = "care_suite_db_token";
    noTokenWarnCount = 0;
    dbFetch = (input, init) => {
...
var healthAssessmentPrintGenerator_exports = {};
__export(healthAssessmentPrintGenerator_exports, {
  generateHealthAssessmentHtml: () => generateHealthAssessmentHtml,
  printHealthAssessment: () => printHealthAssessment
});
module.exports = __toCommonJS(healthAssessmentPrintGenerator_exports);
...
async function getFacilitySettings(forceRefresh = false) {
  if (cachedSettings && !forceRefresh) {
    return cachedSettings;
  }
  const facilityId = getCurrentFacilityId();
  let query = supabase.from("facility_settings").select("facility_name_zh, facility_name_en, facility_address_zh, facility_address_en, facility_phone, facility_fax, logo_data_uri");
  if (facilityId != null) {
...
function parseTextToArray(value) {
...
var formatDate = (dateStr) => formatDisplayDate(dateStr);
var checkboxCells = (checked) => {
  return Array.from({ length: 6 }).map((_, i) => {
    const isChecked = i === 0 && checked ? "checked" : "";
    return `<td><input type="checkbox" class="db-checkbox" ${isChecked}></td>`;
  }).join("");
};
var sideCheckboxCells = (leftChecked, rightChecked) => {
  return Array.from({ length: 6 }).map((_, i) => {
...
var dateCells = (dateStr) => textCells(formatDate(dateStr));
var visionCheckboxCells = (leftValue, rightValue, option) => {
  let cells = "";
  for (let i = 0; i < 6; i++) {
    if (i === 0) {
...
var visionTextCells = (leftValue, rightValue) => {
...
var generateP1 = (assessment, patient, facilityName) => {
  ...
  const isSmoking = (key) => smokingHabit === key;
  const isDrinking = (key) => drinkingHabit === key;
  const limbLeft = Array.isArray(da.limb_movement_left) ? da.limb_movement_left : parseTextToArray(da.limb_movement_left);
  const limbRight = Array.isArray(da.limb_movement_right) ? da.limb_movement_right : parseTextToArray(da.limb_movement_right);
  return `
...
var generateP2 = (assessment, facilityName) => {
  ...
  const bmi = heightNum > 0 && weightNum > 0 ? (weightNum / (heightNum * heightNum)).toFixed(1) : "";
  return `
    <div class="page-p2 print-page">
      <div class="container">
        ...
        <table>
          ...
          <tr style="height: 28px;">
            <th colspan="2">\u9AD4\u8CEA\u6307\u6578 (BMI)<br>(\u516C\u65A4)/(\u7C73)\xB2</th>
            ${textCells(bmi)}
          </tr>
        </table>
...
var generateP3 = (assessment, facilityName) => {
...
var generateHealthAssessmentHtml = (assessment, patient, facilityName) => {
...
var printHealthAssessment = async (assessment, patient) => {
