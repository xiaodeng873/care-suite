...
#### 處方管理 (prescription)：
- 院友姓名
- 藥物名稱（保持原始藥物名稱，包括劑量標示如 paracetamol 500mg；不要附加中文註解、說明或別名）、藥物來源
- 劑型、服用途徑、服用份量（例如 1）、服用單位（例如 粒、毫克）
- 服用次數（每日幾次，數字）、服用日數（數字）、服用時間（HH:MM 陣列，例如 ["08:00","12:00","16:00"]）、處方日期（YYYY-MM-DD，務必填寫處方紙上實際日期，找不到才留空）
- 需要時 (PRN)、備註（只填醫生開立的明確用藥指示備註，不要填入藥房手寫註記、數量標註、換裝說明或其他處理註記；若只有手寫註記則留空）
- 注意：「服用頻率」（如每日/每週/隔日）與「服用次數」分開填寫；「服用時間」與「服用頻率」分開填寫
