...
-- 服務角色擁有完整存取權限（後端系統操作）
CREATE POLICY "service_role_full_access_emergency_log"
  ON public.emergency_operation_log
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
CREATE POLICY "anon_insert_emergency_log"
  ON public.emergency_operation_log
  FOR INSERT
...
CREATE POLICY "authenticated_select_emergency_log"
  ON public.emergency_operation_log
  FOR SELECT
  TO anon, authenticated
  USING (true);
