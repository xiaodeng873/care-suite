...
CREATE INDEX IF NOT EXISTS idx_intake_output_patient_date ON intake_output_records(patient_id, record_date);
CREATE INDEX IF NOT EXISTS idx_intake_output_date ON intake_output_records(record_date);

-- Enable RLS
ALTER TABLE intake_output_records ENABLE ROW LEVEL SECURITY;

-- Create policies (allow all authenticated users for now)
DROP POLICY IF EXISTS "Enable all for authenticated users" ON intake_output_records;
DROP POLICY IF EXISTS "Enable all for authenticated users" ON intake_output_records;
...
DROP TRIGGER IF EXISTS update_intake_output_records_updated_at ON intake_output_records;
DROP TRIGGER IF EXISTS update_intake_output_records_updated_at ON intake_output_records;

CREATE TRIGGER update_intake_output_records_updated_at BEFORE UPDATE ON intake_output_records
  FOR EACH ROW
